...
<!-- lvo-stats:start -->
- **`bsdsocket.library` v4.1 runtime:** 139 total LVO slots (-30 … -858; 121 SFD functions + reserved) generated from Roadshow SDK `sfd/bsdsocket_lib.sfd` via `scripts/gen_lvo_table.py`; **66 BUILT** (vector → IPC → daemon handler, or handled in-library), 55 honest stubs returning exact Roadshow error semantics (`ENOSYS`, `ENXIO`, `NULL`, `NO_RECOVERY`, `FALSE`), 0 BROKEN (marshaled without a daemon handler). Vector-by-vector status and bench PASS coverage: [`src/lib/lib_compat_table.gen.md`](src/lib/lib_compat_table.gen.md) (single source of truth, regenerated by `make python-checks`).
<!-- lvo-stats:end -->
