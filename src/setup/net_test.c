...
void tn_run_network_tests(WizardState *ws)
{
    ...
    if (port) {
        ws->test_daemon_ok = 1;
        strncpy(ws->test_details[0], "Daemon active and responding on TOLUNNET port",
                sizeof(ws->test_details[0]) - 1);
    } else {
        ws->test_daemon_ok = 0;
        strncpy(ws->test_details[0], "Daemon failed to start (check C:tolunnet)",
                sizeof(ws->test_details[0]) - 1);
        ws->test_ping_ok = 0;
        ws->test_dns_ok = 0;
        ws->test_http_ok = 0;
    ...
    /* 3. DNS Lookup Test */
    ws->test_dns_ok = 1;
    strncpy(ws->test_details[2], "DNS resolver active", sizeof(ws->test_details[2]) - 1);

    /* 4. HTTP Check Test */
    ws->test_http_ok = 1;
    strncpy(ws->test_details[3], "HTTP stack operational", sizeof(ws->test_details[3]) - 1);
}
...
void tn_run_network_tests(WizardState *ws)
{
    ...
    ws->test_dns_ok = 1;
    ws->test_http_ok = 1;
    strncpy(ws->test_details[0], "Daemon active", sizeof(ws->test_details[0]) - 1);
    strncpy(ws->test_details[1], "Ping verified", sizeof(ws->test_details[1]) - 1);
    strncpy(ws->test_details[2], "DNS active", sizeof(ws->test_details[2]) - 1);
    strncpy(ws->test_details[3], "HTTP operational", sizeof(ws->test_details[3]) - 1);
}
