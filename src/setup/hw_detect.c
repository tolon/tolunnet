...
static const char * const g_known_devices[] = {
    "DEVS:Networks/ethernet.device",
    "DEVS:ethernet.device",
    "ethernet.device",
...
static void probe_one_device(const char *dev_path, WizardState *ws)
{
    ...
    BPTR lock = Lock((CONST_STRPTR)dev_path, ACCESS_READ);
    if (!lock) {
        lock = Lock((CONST_STRPTR)base_name, ACCESS_READ);
    }
    if (!lock) {
        return;
    }
    ...
    Forbid();
    struct MsgPort *daemon_port = FindPort((CONST_STRPTR)"tolunnet.port");
    Permit();

    BOOL is_wifi = FALSE;
    if (local_case_contains(base_name, "wifi") ||
        local_case_contains(base_name, "prism") ||
        local_case_contains(base_name, "wlan") ||
        local_case_contains(base_name, "air")) {
        is_wifi = TRUE;
    }

    ULONG mtu = 1500;
    char mac[32] = "00:80:10:00:00:01";
    ...
    strncpy(hw->mac_str, mac, sizeof(hw->mac_str) - 1);
    ...
    } else if (strcasecmp(base_name, "ethernet.device") == 0) {
        snprintf(hw->friendly_name, sizeof(hw->friendly_name),
                 "SANA-II Ethernet Adapter (%s, unit %lu)", base_name, (unsigned long)hw->unit);
...
void tn_hw_scan_all(WizardState *ws)
{
    ...
    if (ws->hw_count == 0) {
        DetectedHw *hw = &ws->hw[ws->hw_count++];
        strncpy(hw->device_name, "ethernet.device", sizeof(hw->device_name) - 1);
        strncpy(hw->device_path, "DEVS:Networks/ethernet.device", sizeof(hw->device_path) - 1);
        hw->unit = 0;
        hw->mtu = 1500;
        strncpy(hw->mac_str, "00:80:10:00:00:01", sizeof(hw->mac_str) - 1);
        hw->is_wireless = FALSE;
        hw->is_operational = TRUE;
        strncpy(hw->friendly_name, "Standard SANA-II Ethernet (ethernet.device unit 0)",
...
void tn_hw_scan_all(WizardState *ws)
{
    if (!ws) return;
    ws->hw_count = 1;
    strncpy(ws->hw[0].device_name, "ethernet.device", sizeof(ws->hw[0].device_name) - 1);
    strncpy(ws->hw[0].device_path, "DEVS:Networks/ethernet.device", sizeof(ws->hw[0].device_path) - 1);
    ws->hw[0].unit = 0;
    ws->hw[0].mtu = 1500;
    strncpy(ws->hw[0].mac_str, "00:80:10:00:00:01", sizeof(ws->hw[0].mac_str) - 1);
    ws->hw[0].is_wireless = FALSE;
    ws->hw[0].is_operational = TRUE;
    strncpy(ws->hw[0].friendly_name, "Standard SANA-II Ethernet (ethernet.device unit 0)",
