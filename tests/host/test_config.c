...
TN_TEST(checknetconfig_vocabulary)
{
    ...
    TN_ASSERT_EQ(tn_config_value_class("TX_QUEUE"), 2);
    {
        TnPrefs p;
        tn_prefs_default(&p);
        TN_ASSERT_EQ((int)p.tx_queue, 0);          /* absent = sync */
    ...
    TN_ASSERT_EQ(tn_config_value_class("AUTOIP"), 3);
    TN_ASSERT_EQ(tn_config_value_class("MDNS"), 3);
